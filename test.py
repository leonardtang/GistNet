...
for i in range(np.shape(img_data)[0]):
    ...
    try:
        obj_im = fit_in_square(
            apply_mask(coco, raw_image=load_image(image_id), mask=object_mask, keep='object', crop=0.25), object_dim)
    except:
    ...
    if add == 'gist':
        context_input = prepare_input(
            fit_in_square(apply_mask(coco, raw_image=load_image(image_id), mask=object_mask, keep='context', crop=0),
                          448))
    ...
    if add == 'spotlight_dt':
        ...
        model_pred = model.predict(
            {'object_input': object_input, 'spotlight_input': combined_mask, 'context_input': context_input})
